<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    ...
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %> 
	 ...
	 <c:if test="${ param.login ne null }">
	 		<sql:query dataSource="${db}" var="result">SELECT UserName,Password FROM admin WHERE UserName='${ param.username }' and Password='<%= (new mdhash()).getHashPass(request.getParameter("password"))   %>'</sql:query>
	 		<c:choose>
	 			<c:when test="${ result.rowCount gt 0 }">
	 				<c:set var="alogin" value="${ result.rows[0].UserName }" scope="session"/>
